...
$.getJSON('../emoji.json', (emoji) => {
  const div = Doc.find('.color');

  // emoji 중 skin-tone 골라내기
  emoji.forEach(function (data) {
    if (data.category == 'skin-tone') {
      const emojiCode = twemoji.convert.fromCodePoint(data.codes);
  ...
  $('.skin-tone').bind('click', function () {
    ...
    const list = document.querySelectorAll('.emoji-span');
...
function hideEmoji(name, colors) {
  colors.forEach((color) => {
    ...
    const delEmoji = Doc.findAll(`[title="${title}"]`);
